<?php
/**
 * Handles the account data request form processing.
 *
 * @package     posterno
...
class PNO_Form_Data_Request extends PNO_Form {
	...
	public function __construct() {
		...
		$steps = array(
			...
			'submit_confirmed' => array(
				'name'     => __( 'Data export request confirmation' ),
	...
	public function submit_handler() {
		try {
			...
			if ( empty( $_POST['submit_data-request'] ) ) {
				return;
			}

			if ( ! wp_verify_nonce( $_POST['data-request_nonce'], 'verify_data-request_form' ) ) {
				return;
			}
			...
			$submitted_password = $values['data-request']['password_current'];
			...
			if ( $this->user instanceof WP_User && wp_check_password( $submitted_password, $this->user->data->user_pass, $this->user->ID ) && is_user_logged_in() ) {
				$request_id = wp_create_user_request( $this->user->user_email, 'export_personal_data' );
