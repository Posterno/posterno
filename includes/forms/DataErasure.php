<?php
/**
 * Handles display and processing of the data erasure request form.
 *
 * @package     posterno
...
/**
 * The class of the data erasure request form.
 */
class DataErasure {
	...
	public function hook() {
		add_shortcode( 'pno_request_data_erasure_form', [ $this, 'render' ] );
	...
	protected function getFields() {
		...
		$fields = [
			'password'    => [
				...
				'hint'       => esc_html__( 'Enter your current password to confim erasure of your personal data.', 'posterno' ),
				'required'   => true,
	...
	public function render() {
		...
		if ( is_user_logged_in() ) {
			...
			$message = apply_filters(
				...
				sprintf(
					__( 'You can request cancellation of the data that we have about you. You’ll get an email sent to %s with a link to confirm your request.', 'posterno' ),
			...
			posterno()->templates
				->set_template_data(
					[
						...
						'title'     => esc_html__( 'Request cancellation of your data', 'posterno' ),
						'message'   => $message,
					]
				)
		...
		try {
			...
			if ( $this->form->isValid() ) {
				...
				if ( is_wp_error( $request_id ) ) {
					...
					$message = sprintf( esc_html__( 'A confirmation email has been sent to %s. Click the link within the email to confirm your data erasure request.', 'posterno' ), '<strong>' . $user->data->user_email . '</strong>' );
